...
const seeSubmission=(req,res)=>{
    ...
    let query=`
    select s_assignment_id, marks_given, student_name, st.student_id, assignment_file_name, submission_date, deadline_date, subject_id, datediff(deadline_date,submission_date) as submissionStat from assignment_submission1 asub
...
const semesterRegistrationAssociations=(req,res)=>{

    const processFiles=()=>{
        const tableNames=['assignment_system_faculty','assignment_system_classes','assignment_system_subject','merge','assignment_system_student']
        const parentPath=path.join(__dirname,'..','..','..','my-app-3','public')
        const allFilesProcessed=req.files.map((ele,k)=>{
            return new Promise((resolve,reject)=>{
                const fToC=path.join(parentPath,`${ele.filename}`)
                ...
                papa.parse(file,{   
                    ...
                    complete: function(result){
                        r=result.data
                        query=`insert into ${tableNames[k]} values\n`
                        for(i=1;i<r.length;i++){
                            query=query+'('
                            for(j=0;j<r[i].length-1;j++){
                                query=query+`"`+r[i][j]+`"`+","
                            }
                            query=query+`"`+r[i][j]+`"`+`)${i===r.length-1?";":","}\n`
                        }


                        con.query(query,(error,insertResult)=>{
                            if(error){
        ...
        return Promise.all(allFilesProcessed)
    ...
    const runQuery=async()=>{
        try{
            await processFiles()
            let query=`
            select distinct s.subject_id, class_name, st.student_id from assignment_system_classes c
            inner join merge m on m.class_id=c.class_id
            inner join assignment_system_subject s on m.subject_id=s.subject_id
            inner join assignment_system_student st on c.class_id=st.class_id
            where substr(s.subject_id,length(s.subject_id)) in ("_",st.batch)
            order by c.class_name,s.subject_id;`
            con.query(query,(err,result)=>{
                if(err){
                    console.log(err)
                }
                else{
                    let uploadPath="C:\\Users\\nrm11\\Desktop\\Mini Project\\my-app-3\\public"
                    for(let i=0;i<result.length;i++){      
                        let questionSubFolder=path.join(uploadPath,`${result[i]['subject_id']}\\questions`)   
                        if(!fs.existsSync(questionSubFolder)){
                            fs.mkdirSync(questionSubFolder,{recursive: true})
                        }
                        let folderPath=path.join(uploadPath,`${result[i]['subject_id']}\\answers\\${result[i]['class_name']}\\${result[i]['student_id']}`)
                        if(!fs.existsSync(folderPath)){
                            fs.mkdirSync(folderPath,{recursive: true})
                        }
                    }

                    res.json({status: 200, message: 'Upload and processing Successful'})
                }
            })
...
const uploadFolder=(req,res)=>{
    ...
    //     let folder=subPath[i].slice(subPath[i].indexOf('/')+1)
    //     let subId=folder.slice(0,folder.indexOf('/')) 
    //     let fileName=folder.slice(folder.indexOf("/")+1) 
    //     console.log(subId)
    // }
    res.json({status:200, message: "Wassup"})
}

const uploadFile=(req,res)=>{
    const fToC=path.join(__dirname,'..','..','..','my-app-3','public',`${req.file.filename}`)
    const file=fs.createReadStream(fToC)
    papa.parse(file,{
        header: false,
        dynamicTyping: true,
    ...
    if(rec===recO && recO===conf && conf==="-1"){
        ...
        if(recO!=='0'){
            ...
            if(conf==='p'){
                ...
                inner join merge m on m.merge_id=q.merge_id
                inner join faculty_feedback ff on ff.merge_id=m.merge_id
                inner join feedback_form f on f.feedback_form_id=q.feedback_form_id and f.question_id=q.question_id
                inner join feedback_tracker ft on ft.feedback_form_id=ff.feedback_form_id
                ...
                inner join faculty_feedback ff on ff.merge_id=fs.merge_id
                ...
                inner join feedback_tracker ft on ft.feedback_form_id=ff.feedback_form_id and ft.feedback_form_id=q.feedback_form_id
                where class_id="${rec}"
...
const getFacClassSubList=(req,res)=>{
    ...
    select distinct substr(s.subject_id,-1) as batch, f.faculty_id,fs.merge_id,faculty_name,subject_name,s.subject_id,class_name,c.class_id,abbreviation from merge fs
    inner join faculty_feedback ff on ff.merge_id=fs.merge_id
...
const feedBackOfAStudent=(req,res)=>{
    ...
    select fs.merge_id,faculty_name,subject_name,abbreviation,class_name,subject_type from merge fs
    inner join faculty_feedback ff on ff.merge_id=fs.merge_id
    inner join assignment_system_faculty f on fs.faculty_id=f.faculty_id
    inner join assignment_system_subject s on fs.subject_id=s.subject_id
    inner join assignment_system_classes c on fs.class_id=c.class_id
...
const showSubjectsFaculty=(req,res)=>{
    ...
    let query=`
    select distinct subject_name, abbreviation from assignment_system_subject s
...
const subAndAssgnOfSpecClass=(req,res)=>{
    ...
    order by cast(f_assignment_id as signed integer);
    `    
...
const getAssignmentDetails=(req,res)=>{
    ...
    let query=`
    select st.student_id,class_name,submission_date,assignment_file_name from assignment_submission1 asub
    inner join assignment_system_student st on st.student_id=asub.student_id
    inner join assignment_allocation1 aa on aa.f_assignment_id=asub.f_assignment_id
    inner join merge m on m.merge_id=aa.merge_id
    inner join assignment_system_classes c on c.class_id=st.class_id
